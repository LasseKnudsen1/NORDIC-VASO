function statsjob_testNORDICvaso(version,num_runs,discardFirstVols,num_trials,num_TRperBlock,pROI)
%This function computes PSC and delta maps for VASO and BOLD
%Load data into cell containing all runs both for VASO and BOLD:
counter=0;
for run=1:num_runs
    counter=counter+1;
    %Full timeseries:
    ...
    s=size(Y_VASO_tmp);
    Y_VASO(counter,:,:)=single(reshape(Y_VASO_tmp,s(1)*s(2)*s(3),s(4)));
